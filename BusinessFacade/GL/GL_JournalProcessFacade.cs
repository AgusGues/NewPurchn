    ...
    {
        public class GL_JournalProcessFacade
        {
            ...
            public GL_JournalProcessFacade(GL_JurHead jurhead, ArrayList arrJurDet)
            {
                objGL_JurHead = jurhead;
                arrJurnalDetail = arrJurDet;
            }
            ...
            public string Insert()
            {
                ...
                AbstractTransactionFacade absTrans = new GL_JurHeadFacade(objGL_JurHead);
                ...
                if (intJurHeadID > 0)
                {
                    foreach (GL_JurDet GLJournalDet in arrJurnalDetail)
                    {
                        ...
                        chartbal.Period=objGL_JurHead.Period;
                        chartbal.ChartNo=GLJournalDet.ChartNo ;
                        chartbal.BegBal=0;
                        if (GLJournalDet.IDRAmount>0)
                        {
                            chartbal.DebitTrans=GLJournalDet.IDRAmount;
                            chartbal.CCYDebitTrans=GLJournalDet.CCYAmount ;
                        }
                        else
                        {
                            chartbal.CreditTrans=GLJournalDet.IDRAmount;
                            chartbal.CCYCreditTrans=GLJournalDet.CCYAmount ;
                        }
                        chartbal.DeptCode = GLJournalDet.DeptCode;
                        chartbal.CostCode = GLJournalDet.CostCode;
                        chartbal.CompanyCode = objGL_JurHead.CompanyCode;
                        ...
                        switch (GLJournalDet.JurType)
                        {
                            ...
                            case "AP":
                                ...
                                suppbal.Period = objGL_JurHead.Period;
                                suppbal.SupCode = GLJournalDet.SupCode ;
                                suppbal.BegBal = 0;
                                if (GLJournalDet.IDRAmount > 0)
                                {
                                    suppbal.DebitTrans = GLJournalDet.IDRAmount;
                                    suppbal.CCYDebitTrans = GLJournalDet.CCYAmount;
                                }
                                else
                                {
                                    suppbal.CreditTrans = GLJournalDet.IDRAmount;
                                    suppbal.CCYCreditTrans = GLJournalDet.CCYAmount;
                                }
                            ...
                            case "AR":
                                ...
                                custbal.Period = objGL_JurHead.Period;
                                custbal.CustCode = GLJournalDet.CustCode;
                                custbal.BegBal = 0;
                                if (GLJournalDet.IDRAmount > 0)
                                {
                                    custbal.DebitTrans = GLJournalDet.IDRAmount;
                                    custbal.CCYDebitTrans = GLJournalDet.CCYAmount;
                                }
                                else
                                {
                                    custbal.CreditTrans = GLJournalDet.IDRAmount;
                                    custbal.CCYCreditTrans = GLJournalDet.CCYAmount;
                                }

...
function RequestDepoID() {
    $.ajax({
        url: "LapMutasiStockBJT1.aspx/GetDepoID",
        type: "POST",
...
$("#preview").click(function () {
    ...
    if ($('#harian').is(':checked')) {
        if ($('#meterkubik').is(':checked')) {
            ...
            if ($('#utuh').is(':checked')) {
                $('#modalbj').modal('show');
                RequestBJUtuh(thnbln, strQtyLastMonth, thn)
            } else {
                $('#modalbj').modal('show');
...
function RequestBJ(thnbln, lastmonth, thn) {
    $.ajax({
        url: "LapMutasiStockBJT1.aspx/GetLapMutasiBJT1",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function RequestBJHarian(thnbln, lastmonth, thn, thnbln0) {
    $.ajax({
        url: "LapMutasiStockBJT1.aspx/GetLapMutasiBJT1Harian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function RequestBJM3(thnbln, lastmonth, thn) {
    $.ajax({
        ...
        success: function (data) {
            ...
            delete data.results
            data = $.parseJSON(data);
            DrawTableBJT1M3(data);
            DrawTableBJT1M3Print(data);
        },
        complete: function (data) {
            $("#loading").hide($.unblockUI());
        }
...
function RequestBJM3Harian(thnbln, lastmonth, thn, thbln0) {
    $.ajax({
        url: "LapMutasiStockBJT1.aspx/GetLapMutasiBJT1Harian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, thbln0: thbln0 }),
        beforeSend: function () {
            $("#loading").show($.blockUI({ message: null }));
        },
        success: function (data) {
...
function RequestBJUtuh(thnbln, lastmonth, thn) {
    $.ajax({
        url: "LapMutasiStockBJT1.aspx/GetLapMutasiBJT1Konversi",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn }),
...
function DrawTableBJT1(data) {
    ...
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang Jadi Tahap 1, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebjt1" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
...
function DrawTableBJT1Print(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    textHTML += '<table id="tablebjt1print" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;">';
    ...
    for (var i = 0; i < data.length; i++) {
        textHTML += '<tr>';
        textHTML += '<td style="width:150px; border-bottom: 1px dotted;">' + data[i].Partno + '</td>';
    ...
    $("#tbjt1print").html(textHTML);
}


function DrawTableBJT1M3(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    var m3 = 0;
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang Jadi Tahap 1, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebjt1m3" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
    textHTML += '<th rowspan="2" style="vertical-align: middle; text-align: center;">Partno</th>';
    textHTML += '<th colspan="7" style="vertical-align: middle; text-align: center;">Penerimaan Dari</th>';
    ...
    $("#tbjt1m3").html(textHTML);
    $("#tablebjt1m3").DataTable({
        "pageLength": 50
    });
}

function DrawTableBJT1M3Print(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    var outadjust = 0;
    var saldoakhir = 0;
    textHTML += '<table id="tablebjt1m3print" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;">';
...
function fnExcelReport() {
    ...
    tab = document.getElementById('tablebjt1print');
...
function printData() {
    var divToPrint = document.getElementById("tablebjt1print");
    newWin = window.open("");
...
function fnExcelReportm3() {
    var tab_text = "<table border='2px'><tr>";
    var textRange; var j = 0;
    tab = document.getElementById('tablebjt1m3print');
...
function printDatam3() {
    var divToPrint = document.getElementById("tablebjt1m3print");
    newWin = window.open("");
    newWin.document.write(divToPrint.outerHTML);
    newWin.print();
    newWin.close();

...
$("#modalbp").on("hidden.bs.modal", function () {
    $("#tbp").html("");
});

$("#modalbpm3").on("hidden.bs.modal", function () {
    $("#tbpm3").html("");
});
...
function RequestDepoID() {
    $.ajax({
        url: "LapMutasiStockBP.aspx/GetDepoID",
        type: "POST",
...
$("#preview").click(function () {
    ...
    if ($('#harian').is(':checked')) {
        if ($('#meterkubik').is(':checked')) {
            $('#modalbpm3').modal('show');
            RequestBPM3Harian(thnbln, strQtyLastMonth, thn, thnblnhari, dept);
        }
        else {
            $('#modalbp').modal('show');
            RequestBPHarian(thnbln, strQtyLastMonth, thn, thnblnhari, dept);
        }
    } else {
        if ($('#meterkubik').is(':checked')) {
            $('#modalbpm3').modal('show');
            RequestBPM3(thnbln, strQtyLastMonth, thn, dept);
        }
        else {
            $('#modalbp').modal('show');
            RequestBP(thnbln, strQtyLastMonth, thn, dept);
        }
    }
});


function RequestBP(thnbln, lastmonth, thn, dept) {
    $.ajax({
        url: "LapMutasiStockBP.aspx/GetLapMutasiBP",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, dept: dept }),
        beforeSend: function () {
...
function RequestBPHarian(thnbln, lastmonth, thn, thnblnhari, dept) {
    $.ajax({
        url: "LapMutasiStockBP.aspx/GetLapMutasiBPHarian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, thnblnhari: thnblnhari, dept: dept }),
        beforeSend: function () {
            $("#loading").show($.blockUI({ message: null }));
        },
        success: function (data) {
...
function RequestBPM3(thnbln, lastmonth, thn, dept) {
    $.ajax({
        url: "LapMutasiStockBP.aspx/GetLapMutasiBP",
        type: "POST",
        contentType: "application/json; charset=utf-8",
...
function RequestBPM3Harian(thnbln, lastmonth, thn, thnblnhari, dept) {
    $.ajax({
        url: "LapMutasiStockBP.aspx/GetLapMutasiBPHarian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
...
function DrawTableBP(data) {
    ...
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang Potong, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebp" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
...
function DrawTableBPPrint(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    var outbsm3 = 0;
    var outadjustm3 = 0;
    textHTML += '<table id="tablebpprint" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;">';

    textHTML += '<thead>';
    textHTML += '<tr>';
    textHTML += '<td colspan="16" style="vertical-align: middle; text-align: center; border: none;"><h4>Laporan Rekap Mutasi Stock Barang Jadi Tahap 1, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4></th>';
    textHTML += '</tr>';
    ...
    $("#tbpprint").html(textHTML);
}

function DrawTableBPM3(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang Potong, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebpm3" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
    textHTML += '<th rowspan="2" style="vertical-align: middle; text-align: center;">Partno</th>';
    textHTML += '<th colspan="7" style="vertical-align: middle; text-align: center;">Penerimaan Dari</th>';
...
function DrawTableBPM3Print(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    var saldoakhir = 0;
    var m3 = 0;
    textHTML += '<table id="tablebpm3print" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;">';

    textHTML += '<thead>';
    textHTML += '<tr>';
    textHTML += '<td colspan="15" style="vertical-align: middle; text-align: center; border: none;"><h4>Laporan Rekap Mutasi Stock Barang Jadi Tahap 1, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4></th>';
    textHTML += '</tr>';
    ...
    $("#tbpm3print").html(textHTML);
}

function printData() {
    var divToPrint = document.getElementById("tablebpprint");
    newWin = window.open("");
...
function fnExcelReport() {
    ...
    tab = document.getElementById('tablebpprint');
...
function printDatam3() {
    var divToPrint = document.getElementById("tablebpm3print");
    newWin = window.open("");
    newWin.document.write(divToPrint.outerHTML);
    newWin.print();
    newWin.close();
...
function fnExcelReportm3() {
    var tab_text = "<table border='2px'><tr>";
    var textRange; var j = 0;
    tab = document.getElementById('tablebpm3print');

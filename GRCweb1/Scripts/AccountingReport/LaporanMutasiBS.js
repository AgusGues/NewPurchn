...
$("#modalbsm3").on("hidden.bs.modal", function () {
    $("#tbsm3").html("");
});

function RequestDepoID() {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetDepoID",
        type: "POST",
...
$("#preview").click(function () {
    ...
    if ($('#harian').is(':checked')) {
        if ($('#meterkubik').is(':checked')) {
            if ($('#utuh').is(':checked')) {
                RequestBSM3Harian(thnbln, strQtyLastMonth, thn, flag)
                $('#modalbsm3').modal('show');
            }
            else {
                RequestBSM3Harian(thnbln, strQtyLastMonth, thn, thnblnhari, bs, flag)
                $('#modalbsm3').modal('show');
            }
        }
        else {
            if ($('#utuh').is(':checked')) {
                RequestBSKonversi(thnbln, strQtyLastMonth, thn, thnblnhari, bs, flag)
                $('#modalbs').modal('show');
            } else {
                RequestBSHarian(thnbln, strQtyLastMonth, thn, thnblnhari, bs, flag)
                $('#modalbs').modal('show');
            }
        }
    } else {
        if ($('#meterkubik').is(':checked')) {
            if ($('#utuh').is(':checked')) {
                RequestBSM3(thnbln, strQtyLastMonth, thn, bs, flag)
                $('#modalbsm3').modal('show');
            } else {
                RequestBSM3(thnbln, strQtyLastMonth, thn, bs, flag)
                $('#modalbsm3').modal('show');
            ...
            if ($('#utuh').is(':checked')) {
                RequestBSKonversi(thnbln, strQtyLastMonth, thn, flag)
                $('#modalbs').modal('show');
            } else {
                RequestBS(thnbln, strQtyLastMonth, thn, bs, flag)
                $('#modalbs').modal('show');
            }
        }
    }
...
function RequestBS(thnbln, lastmonth, thn, bs, flag) {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetLapMutasiBS",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, bs:bs, flag:flag }),
        beforeSend: function () {
...
function RequestBSHarian(thnbln, lastmonth, thn, thnbln0, bs, flag) {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetLapMutasiBSHarian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, thnbln0: thnbln0, bs:bs, flag:flag }),
        beforeSend: function () {
            $("#loading").show($.blockUI({ message: null }));
        },
        success: function (data) {
...
function RequestBSM3(thnbln, lastmonth, thn, bs, flag) {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetLapMutasiBS",
        type: "POST",
        contentType: "application/json; charset=utf-8",
...
function RequestBSM3Harian(thnbln, lastmonth, thn, thnbln0, bs, flag) {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetLapMutasiBSHarian",
        type: "POST",
        contentType: "application/json; charset=utf-8",
...
function RequestBSKonversi(thnbln, lastmonth, thn, flag) {
    $.ajax({
        url: "LapMutasiStockBS.aspx/GetLapMutasiBSKonversi",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ thnbln: thnbln, lastmonth: lastmonth, thn: thn, flag: flag }),
        beforeSend: function () {
            $("#loading").show($.blockUI({ message: null }));
        },
        success: function (data) {
...
function DrawTableBS(data) {
    ...
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang BS, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebs" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
...
function DrawTableBSPrint(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    textHTML += '<table id="tablebsprint" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;">';
    ...
    $("#tbsprint").html(textHTML);
}

function DrawTableBSM3(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    textHTML += '<h4>Laporan Rekap Mutasi Stock Barang BS, Periode : ' + $("#bulan option:selected").text() + ' ' + $('#tahun').val() + '</h4>';
    textHTML += '<table id="tablebsm3" class="table table-striped table-bordered table-hover display nowrap" style="width: 100%" border="4">';
    textHTML += '<thead>';
    textHTML += '<tr>';
    textHTML += '<th rowspan="2" style="vertical-align: middle; text-align: center;">Partno</th>';
    textHTML += '<th colspan="7" style="vertical-align: middle; text-align: center;">Penerimaan Dari</th>';
...
function DrawTableBSM3Print(data) {
    var textHTML = '';
    var insaldoawal = 0;
    var inwip = 0;
    var inbp = 0;
    ...
    var saldoakhir = 0;
    var m3 = 0;
    textHTML += '<table id="tablebsm3print" class="table" style="width: 100%; border-collapse: collapse; border: thin solid black;>';
    ...
    if (depo == 1) {
        ...
        textHTML += '<td colspan="3" style="vertical-align: middle; text-align: center; border: none;"></th>';
        textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;"><b>Citeureup, ' + $.datepicker.formatDate('dd-mm-yy', new Date()); +' </b></th>'
    } else if (depo == 7) {
        textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;"></th>';
        textHTML += '<td colspan="3" style="vertical-align: middle; text-align: center; border: none;"></th>';
        textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;"><b>Karawang, ' + $.datepicker.formatDate('dd-mm-yy', new Date()); +' </b></th>'
    } else if (depo == 13) {
        textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;"></th>';
        textHTML += '<td colspan="3" style="vertical-align: middle; text-align: center; border: none;"></th>';
        textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;"><b>Jombang, ' + $.datepicker.formatDate('dd-mm-yy', new Date()); +' </b></th>'
    }
    textHTML += '</tr>';

    textHTML += '<tr>';
    textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;">Diketahui</th>';
    textHTML += '<td colspan="3" style="vertical-align: middle; text-align: center; border: none;">Disetujui</th>';
    textHTML += '<td colspan="4" style="vertical-align: middle; text-align: center; border: none;">Dibuat</th>';
    textHTML += '</tr>';

    textHTML += '<tr>';
    textHTML += '<td  colspan="4" style="vertical-align: middle; text-align: center; border: none;"></td>';
    textHTML += '<td  colspan="3" style="vertical-align: middle; text-align: center; border: none;"></td>';
    textHTML += '<td  colspan="4" style="vertical-align: middle; text-align: center; border: none;"></td>';
    textHTML += '</tr>';
    ...
    textHTML += '<td  colspan="3" style="vertical-align: middle; text-align: center; padding-top: 70px; border: none;">(______________)</td>';
    textHTML += '<td  colspan="4" style="vertical-align: middle; text-align: center; padding-top: 70px; border: none;">(______________)</td>';
    textHTML += '</tr>';

    textHTML += '</table>';

    $("#tbsm3print").html(textHTML);
}

function printData() {
    var divToPrint = document.getElementById("tablebsprint");
    newWin = window.open("");
...
function fnExcelReport() {
    ...
    tab = document.getElementById('tablebsprint');
...
function fnExcelReportm3() {
    var tab_text = "<table border='2px'><tr>";
    var textRange; var j = 0;
    tab = document.getElementById('tablebsm3print');
...
function printDatam3() {
    var divToPrint = document.getElementById("tablebsm3print");
    newWin = window.open("");
    newWin.document.write(divToPrint.outerHTML);
    newWin.print();
    newWin.close();
